...
开机联网双保险
  设备上电后由独立线程等待固件网络初始化完成，再优先尝试已保存的 Wi-Fi；不会阻塞摄像头和识别初始化。
  户外可把手机热点名称和密码设置成与公司 Wi-Fi 完全相同，设备会直接接入。
  客户端凭据独立保存在 /root/snail_egg/wifi_sta.ssid 和 wifi_sta.pass，不会被热点配置覆盖。
  Wi-Fi 不存在、连接失败或运行中断线时，设备自动建立开放热点：
...
屏幕应急联网入口
  ...
  Wi-Fi 连接失败或运行中断线时会自动回滚到设备热点，不会永久失联。
