...
设备已经设置过自启动、只想同步新代码时，明确传入当前应用 ID。此命令会同时更新
`/root/snail_egg` 和真正的 `/maixapp/apps/<应用ID>`，并打印两份文件的 MD5；随后重启
即可加载新版本。当前应用 ID 如果是 `cdh1_`，命令如下：

```powershell
python scripts\maix_remote.py --host 192.168.10.108 deploy --skip-models --app-id cdh1_
python scripts\maix_remote.py --host 192.168.10.108 reboot
```
