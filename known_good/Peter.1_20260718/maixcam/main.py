...
MODEL = "/root/models/snail_eggs_yolo11n_320x224.mud"
USE_FAST_MODEL = True
FAST_MODEL_IS_YOLO11 = True
CONF_TH = 0.10 if USE_FAST_MODEL else 0.20
IOU_TH = 0.35
MAX_TARGETS = 32
MIN_MODEL_CONF = 0.10 if USE_FAST_MODEL else 0.20
STRONG_MODEL_CONF = 0.32 if USE_FAST_MODEL else 0.38
...
GIMBAL_CONTROL_HZ = 12.0
GIMBAL_DEADZONE_X = 0.050
GIMBAL_DEADZONE_Y = 0.060
GIMBAL_SLOW_MAX_STEP_DEG = 0.20
GIMBAL_SLOW_STEP_CHANGE_DEG = 0.035
GIMBAL_SLOW_FULL_SPEED_ERROR = 0.32
GIMBAL_SLOW_ERROR_FILTER_TAU_S = 0.28
GIMBAL_OUTPUT_MAX_STEP_DEG = 0.15
...
class Gimbal:
    ...
    def _writer_loop(self):
        while not self._stop:
            ...
            if pending != self._applied:
                try:
                    next_pan = self._approach(self._applied[0], pending[0], GIMBAL_OUTPUT_MAX_STEP_DEG)
                    next_tilt = self._approach(self._applied[1], pending[1], GIMBAL_OUTPUT_MAX_STEP_DEG)
                    self._apply_offsets(next_pan, next_tilt)
                except Exception as e:
                    print("GIMBAL_PWM_ERROR,%s" % e)
            safe_sleep(1.0 / GIMBAL_CONTROL_HZ)
...
manual_pan_output = 0.0
manual_tilt_output = 0.0
manual_output_clock = pytime.time()
profile_count = 0
...
while not app.need_exit():
        ...
        if action == "clear":
            ...
            if selected_obj is not None:
                ...
                if web_control is not None:
                    web_control.begin_fixed_target(
                        current_pan_offset,
                        current_tilt_offset,
                        selected_goal_pan,
                        selected_goal_tilt,
                ...
                web_pan = selected_goal_pan
                web_tilt = selected_goal_tilt
                web_mode = "manual"
                selection_changed = True
            else:
                clear_target_lock()
    ...
    current_pan_offset = float(getattr(gimbal, "pan_offset", 0.0)) if gimbal is not None else 0.0
    current_tilt_offset = float(getattr(gimbal, "tilt_offset", 0.0)) if gimbal is not None else 0.0
    if web_mode == "selected" and fresh_aim_present and primary_obj is not None:
        ...
        selected_last_box_pan = current_pan_offset
        selected_last_box_tilt = current_tilt_offset
    if web_mode == "selected" and selected_loss_detections >= SELECTED_LOSS_CONFIRM_DETECTIONS:
    ...
    if web_mode == "selected" and aim_waypoint_planner.take_centered_ready():
        current_pan_offset = float(getattr(gimbal, "pan_offset", 0.0)) if gimbal is not None else 0.0
        current_tilt_offset = float(getattr(gimbal, "tilt_offset", 0.0)) if gimbal is not None else 0.0
        if web_control is not None:
    ...
    if gimbal_tracker is not None:
        if web_estop or web_mode == "hold":
            ...
            now = pytime.time()
            if last_control_mode != "manual":
                # Take over from the actual current command. Never jump from
                # the web page's stale zero to the moving gimbal position.
                manual_pan_output = float(getattr(gimbal, "pan_offset", 0.0))
                manual_tilt_output = float(getattr(gimbal, "tilt_offset", 0.0))
            dt = max(0.001, min(0.12, now - manual_output_clock))
            manual_output_clock = now
            manual_max_step = 2.0 * dt
            manual_pan_output += max(-manual_max_step, min(manual_max_step, web_pan - manual_pan_output))
            manual_tilt_output += max(-manual_max_step, min(manual_max_step, web_tilt - manual_tilt_output))
            manual_pan_output = clamp_pan_offset(manual_pan_output)
            manual_tilt_output = clamp_tilt_offset(manual_tilt_output)
            gimbal.set_offsets(manual_pan_output, manual_tilt_output, "WEB")
            control_status = "WEB,MANUAL,PAN,%.1f,TILT,%.1f" % (manual_pan_output, manual_tilt_output)
    ...
    profile_display_s += profile_display_done - profile_pre_display
    if profile_count >= PROFILE_EVERY_N_FRAMES:
        prof_line = (
            "PROF,%d,FRAMES,%d,DETECTS,%d,READ_MS,%.2f,DOT_MS,%.2f,"
            "DETECT_MS,%.2f,CONTROL_DRAW_MS,%.2f,DISPLAY_MS,%.2f"
            % (
                frame_id,
                profile_count,
