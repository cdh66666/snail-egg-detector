...
- 网络：设备上电后由独立线程等待固件网络初始化完成，再尝试已保存的公司 Wi-Fi；户外把手机热点改成相同名称和密码即可复用。连接失败或运行中断线时，自动恢复开放热点 `SnailEgg-MaixCAM`，固定地址为 `http://192.168.66.1:8000/?token=maixcam`。客户端凭据独立保存在 `/root/snail_egg/wifi_sta.*`，不会被固件热点配置覆盖。热点守护线程持续检查无线接口、AP 和 DHCP，异常时自动重建；屏幕右下角保留 `WIFI/HOTSPOT` 触摸入口，USB 有线维护地址固定为 `http://10.178.38.1:8000/?token=maixcam`。
