...
开启真实舵机跟踪前，必须只连接安全的舵机负载，并确认当前实测接线为水平 A18/PWM6、俯仰 A19/PWM7。控制器包含稳定帧门槛、当前帧检测门槛、图像中心死区、5 Hz 更新、单次最大 0.5° 变化和 45° 绝对偏移限幅。预测框只用于保持画面和 ID 连续，不会驱动舵机。紧急禁用可以在设备上创建：
