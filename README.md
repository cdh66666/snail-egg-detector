...
- 网络：设备上电默认启动开放热点 `SnailEgg-MaixCAM`，地址固定为 `http://192.168.66.1:8000/?token=maixcam`。网页提供公司 Wi-Fi 名称和密码配置窗口，保存后立即连接并显示获得的 `network_ip`；USB 有线维护地址固定为 `http://10.178.38.1:8000/?token=maixcam`。
- 现场调参：网页“识别阈值”按钮可在 `0.05..0.30` 范围内以 `0.02` 步进调整 YOLO 置信度；默认 `0.10`，低光或远距离可逐步降低，误检增加时再提高。
