...
红点暂时不可见时保持当前舵机角度；持续失效后回到经过标定的固定参考点搜索，不盲目继续移动。红点继电器按整幅视野判断：视野内任意有效 YOLO 卵团连续 3 帧才开启，只有整幅视野没有有效卵团时关闭。`/root/snail_egg/enable_aim_scan` 会在红点到达当前点并稳定后，才切换到下一个轮廓点。
