...
创建 `screen_tracking_test` 标志进行近距离桌面调试时，程序使用当前约 `0.35 m` 测试台实测的 `(329,326)`。这只用于屏幕追踪演示；删除标志后自动恢复 1 m 的 `(324,270)`。

为避免可见瞄准点被模型误认为卵团，屏幕模式使用 `0.08` 阈值，并排除瞄准点 `±32 px` 内且不大于 `52x56 px` 的小框。更大的真实卵团不会被该规则过滤。
